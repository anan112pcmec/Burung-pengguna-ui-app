...
{#snippet Card1()}
    <div class="p-1.5 grid grid-rows-[35%_65%] {Full} gap-1.5 text-slate-800">
        <!-- Baris Atas: Info Ringkas / Statik -->
        <div class="p-1.5 space-x-1.5 grid grid-cols-[35%_65%] bg-white border border-slate-200 rounded-lg shadow-sm">
            <!-- Kiri atas: Kategori Terpopuler -->
            <div class="bg-slate-900 rounded flex flex-col justify-center items-center text-white p-1">
                <span class="text-[9px] uppercase tracking-wider opacity-80">Trend</span>
                <span class="text-xs font-bold tracking-tight">{statsCard1.trendRank}</span>
            </div>
            <!-- Kanan atas: Ringkasan Aktivitas -->
            <div class="bg-slate-50 border border-slate-100 rounded p-1 flex flex-col justify-between">
                <div class="flex justify-between items-center">
                    <span class="text-[9px] font-medium text-slate-400 uppercase tracking-wider">{statsCard1.category}</span>
                    <span class="h-1.5 w-1.5 rounded-full bg-slate-400"></span>
                </div>
                <div class="flex justify-between items-baseline">
                    <span class="text-xs font-semibold text-slate-700">Barang Baru</span>
                    <span class="text-[9px] text-slate-500 font-medium">{statsCard1.newItemsCount}</span>
                </div>
            </div>
        </div>

        <!-- Baris Bawah: Display List Produk Minimalis -->
        <div class="p-0.5 space-x-1.5 grid grid-cols-[35%_65%]">
            <!-- Kiri bawah: Mini Preview / Placeholder Gambar -->
            <div class="bg-white border border-slate-200 rounded-lg p-1.5 flex flex-col justify-between shadow-sm">
                <div class="w-full aspect-square bg-slate-50 border border-slate-100 rounded flex items-center justify-center relative overflow-hidden">
                    <!-- Placeholder Icon Clean -->
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-slate-400">
                        <path stroke-linecap="round" stroke-linejoin="round" d="m2.25 15.75 5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 0 0 1.5-1.5V6a1.5 1.5 0 0 0-1.5-1.5H3.75A1.5 1.5 0 0 0 2.25 6v12a1.5 1.5 0 0 0 1.5 1.5Zm10.5-11.25h.008v.008h-.008V8.25Zm.375 0a.375 0 1 1-.75 0 .375 0 0 1 .75 0Z" />
                    </svg>
                </div>
                <div class="mt-1">
                    <p class="text-[9px] text-slate-400 truncate">{mainProductCard1.category}</p>
                    <p class="text-[11px] font-bold text-slate-900 truncate">{mainProductCard1.price}</p>
                </div>
            </div>

            <!-- Kanan bawah: List Item Utama -->
            <div class="bg-white border border-slate-200 rounded-lg p-1.5 flex flex-col justify-between shadow-sm">
                    ...
                    {#each featuredProductsCard1 as item (item.id)}
                        <div class="flex items-center justify-between p-1 rounded bg-slate-50 border border-slate-100">
                            <div class="flex items-center gap-1.5 min-w-0">
                                ...
                                <span class="text-[10px] font-medium text-slate-600 truncate">{item.name}</span>
                            </div>
                            <span class="text-[10px] font-bold text-slate-900 flex-shrink-0">{item.status}</span>
                        </div>
                    {/each}
                </div>
            </div>
...
{#snippet Card2()}
    <div class="grid grid-rows-[80%_20%] {Full}">
        <div>
            <DotLottieSvelte
        ...
        <!-- Div kedua: Hanya kata-kata tentang customer service -->
        <div class="flex items-center justify-center px-4 text-center">
            <p class="text-[11px] font-medium text-slate-500 mb-10 tracking-wide leading-relaxed">
...
{#snippet Card3()}
    <!-- Menggunakan layout 3 kolom asimetris yang memecah kejenuhan kotak Card1 -->
    <div class="p-2 grid grid-cols-[55%_45%] {Full} gap-2 text-slate-800">
        
        <!-- KOLOM KIRI: Big Bold Metric Card (Sengaja dibikin dominan & lega) -->
        <div class="bg-white border border-slate-200 rounded-xl p-3 flex flex-col justify-between shadow-sm relative overflow-hidden">
            <!-- Efek aksen garis minimalis di pojok -->
            <div class="absolute top-0 left-0 w-1 h-full bg-slate-950"></div>
            
            <div class="flex justify-between items-center pl-1">
                <span class="text-[9px] font-bold uppercase tracking-widest text-slate-400">Performa</span>
                <span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[8px] font-medium bg-slate-100 text-slate-800 border border-slate-200">
                    Live
                </span>
            </div>
            
            <div class="my-auto pl-1">
                <h2 class="text-4xl font-black tracking-tighter text-slate-900 leading-none">
                    {speedData.mainMetric}
                </h2>
                <p class="text-[10px] text-slate-500 font-medium mt-1 tracking-wide">
                    {speedData.subText}
                </p>
            </div>
        </div>

        <!-- KOLOM KANAN: Split Stack Grid (2 Kotak Kecil Vertikal) -->
        <div class="grid grid-rows-2 gap-2">
            <!-- Kanan Atas: Mini Ping Indicator -->
            <div class="bg-slate-950 text-white rounded-xl p-2.5 flex items-center justify-between shadow-sm">
                <div class="flex flex-col">
                    <span class="text-[8px] uppercase tracking-wider text-slate-400">Ping Jaringan</span>
                    <span class="text-sm font-bold tracking-tight mt-0.5">{speedData.ping}</span>
                </div>
                <!-- Icon Gelombang/Sinyal Minimalis -->
                <div class="w-6 h-6 rounded-lg bg-white/10 flex items-center justify-center flex-shrink-0">
                    <span class="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-ping absolute"></span>
                    <span class="w-1.5 h-1.5 bg-emerald-400 rounded-full relative"></span>
                </div>
            </div>

            <!-- Kanan Bawah: Uptime Progress -->
            <div class="bg-white border border-slate-200 rounded-xl p-2.5 flex flex-col justify-between shadow-sm">
                <div class="flex justify-between items-center">
                    <span class="text-[8px] font-bold uppercase tracking-wider text-slate-400">Uptime</span>
                    <span class="text-[9px] font-bold text-slate-900">{speedData.status}</span>
                </div>
                <!-- Simbol Garis Speed Minimalis -->
                <div class="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden mt-1.5 border border-slate-200">
                    <div class="bg-slate-950 h-full w-[95%] rounded-full"></div>
                </div>
            </div>
        </div>
...
>
    ...
    >
       ...
       <div
            class="absolute inset-0 backface-hidden border border-zinc-900/10 p-6 bg-gradient-to-br w-[95%] from-slate-100/60 via-slate-100/40 to-white/30 {navbarState.searching ? "": "backdrop-blur-xl"} rounded-2xl grid grid-rows-[50%_50%] overflow-hidden shadow-xl shadow-slate-900/5"
            ...
            <div>
                <div class="grid grid-cols-[15%_75%_10%] h-full">
                    ...
                    <div class="flex flex-col justify-between pb-6">
                        ...
                        <h4 class="text-left font-display text-lg/tight text-slate-900 font-semi-bold font-signature leading-relaxed">
                        ...
                        <div class="space-x-4 flex justify-left mt-4 mb-10">
                            ...
                            <button
                                class="group relative overflow-hidden text-white bg-slate-900 px-4 py-2 rounded-lg transition duration-300 shadow-md shadow-slate-900/10 hover:shadow-lg hover:bg-slate-800"
                            >
                              <span class="relative z-10" onclick={() => {
                                    window.scrollBy({
                                        top: 478, 
                                        behavior: 'smooth' 
                                    });
                            ...
                            <button
                                onclick={() => rotate = true}
                                class="relative px-4 py-2 rounded-lg border border-slate-900/10 hover:border-slate-900 transition duration-300 hover:bg-slate-900 hover:text-white"
                            >
                                Bergabung
                            </button>
        ...
        <div
            class="absolute inset-0 backface-hidden rotate-y-180 bg-slate-50 border border-zinc-900/10 rounded-2xl flex flex-col justify-center py-4 sm:px-6 lg:px-8 overflow-y-auto"
            style:z-index={rotate && !navbarState.searching ? 20 : 10}
        >
            <div class="sm:mx-auto sm:w-full sm:max-w-sm px-4">

                <h1 class="text-left font-display font-semibold text-2xl text-slate-900 tracking-tight">
                    Login
                </h1>
                ...
                <div class="mt-3">
                    <form class="space-y-2.5" onsubmit={(e) => e.preventDefault()}>
                        ...
                        <div>
                            <label class="block text-xs font-medium text-slate-600 mb-0.5">
                                Email
                            </label>

                            <input 
                                type="email"
                                class="w-full rounded-md border border-slate-300 p-2 text-sm text-slate-950 placeholder-slate-400 focus:outline-none focus:border-slate-900 focus:ring-1 focus:ring-slate-900 transition"
                        ...
                        <div class="grid grid-cols-2 gap-3 pt-1">
                            ...
                            <button
                                type="submit"
                                class="w-full bg-slate-900 text-white hover:bg-slate-800 transition duration-300 px-3 py-2 text-xs font-medium rounded-md text-center shadow-sm"
                            >
                                Login
                            </button>
                            ...
                            <a
                                href="/signup"
                                class="w-full text-slate-900 bg-white border border-slate-300 px-3 py-2 text-xs font-medium rounded-md transition duration-300 text-center hover:bg-slate-50 block"
                            >
                                Sign Up
                            </a>
                    ...
                    <div class="flex justify-start mt-4 pt-2 border-t border-slate-200">
                        ...
                        <button
                            ...
                            class="text-slate-500 bg-transparent px-2 py-1 text-xs font-medium rounded-md transition duration-300 hover:text-slate-900 hover:bg-slate-100 flex items-center gap-1.5"
                        >
